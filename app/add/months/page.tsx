            ...
            {months &&
              months.map((item, index) => (
                <div
                  ...
                  className={`flex justify-between rounded-md shadow-md shadow-cyan-500/50 mx-1 p-1 bg-neutral-800 border border-black 
              ${
                item.active
  ...
  const handleDeleteMonth = async (month) => {
    ...
    try {
      ...
      openInfoModal({
        ...
        message: `The ${month.month} month is deleted (@_-)`,
        isSuccess: true,
      });
    } catch (error) {
      closeModal();
      openInfoModal({
        ...
        message: "somthing went wrong check network!! and try agian",
        isSuccess: false,
      });
    ...
    >
      <div className="bg-gray-800 rounded-lg shadow mx-4 w-full border border-gray-500">
        ...
        <p className="text-gray-300 m-2">
          Are you sure you want to delete this month {MonthData.month}?
        </p>
